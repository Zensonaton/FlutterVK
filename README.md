...
Благодаря магии фреймворка [Flutter](https://flutter.dev/), приложение Flutter VK может запускаться сразsу на множества платформах. В данный момент, Flutter VK можно запустить на следующих платформах:
...
3. Запустите приложение. Если Вы пользуетесь VS Code, то нажмите F5, и приложение запустится в Debug-режиме через некоторое время. В ином случае, воспользуйтесь:
...
> [!NOTE]
> Debug-версии билдов имеют отвратительную производительность, а так же имеют огромный размер `.exe`/`.apk`-файлов, и это нормально.
